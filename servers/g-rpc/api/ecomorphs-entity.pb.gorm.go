...
type EcomorphsEntityORM struct {
	...
	Description string
	EcomorphId  *string
	Ecomorphs   *EcomorphORM `gorm:"foreignKey:EcomorphId;references:Id"`
...
func (m *EcomorphsEntity) ToORM(ctx context.Context) (EcomorphsEntityORM, error) {
	...
	if m.Ecomorphs != nil {
		tempEcomorphs, err := m.Ecomorphs.ToORM(ctx)
		if err != nil {
			return to, err
		}
		to.Ecomorphs = &tempEcomorphs
	}
	if m.UserId != nil {
...
func (m *EcomorphsEntityORM) ToPB(ctx context.Context) (EcomorphsEntity, error) {
	...
	to.Description = m.Description
	if m.Ecomorphs != nil {
		tempEcomorphs, err := m.Ecomorphs.ToPB(ctx)
		if err != nil {
			return to, err
		}
		to.Ecomorphs = &tempEcomorphs
...
func DefaultCreateEcomorphsEntity(ctx context.Context, in *EcomorphsEntity, db *gorm.DB) (*EcomorphsEntity, error) {
	...
	if err = db.Omit().Preload("Ecomorphs").Create(&ormObj).Error; err != nil {
		return nil, err
	}
...
func DefaultStrictUpdateEcomorphsEntity(ctx context.Context, in *EcomorphsEntity, db *gorm.DB) (*EcomorphsEntity, error) {
	...
	if err = db.Omit().Preload("Ecomorphs").Save(&ormObj).Error; err != nil {
		return nil, err
	}
...
func DefaultApplyFieldMaskEcomorphsEntity(ctx context.Context, patchee *EcomorphsEntity, patcher *EcomorphsEntity, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*EcomorphsEntity, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if !updatedEcomorphs && strings.HasPrefix(f, prefix+"Ecomorphs.") {
			updatedEcomorphs = true
			if patcher.Ecomorphs == nil {
				patchee.Ecomorphs = nil
				continue
			}
			if patchee.Ecomorphs == nil {
				patchee.Ecomorphs = &Ecomorph{}
			}
			if o, err := DefaultApplyFieldMaskEcomorph(ctx, patchee.Ecomorphs, patcher.Ecomorphs, &field_mask.FieldMask{Paths: updateMask.Paths[i:]}, prefix+"Ecomorphs.", db); err != nil {
				return nil, err
			} else {
				patchee.Ecomorphs = o
			}
